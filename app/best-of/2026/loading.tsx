export default function BestOf2026Loading() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-20 sm:py-28">
      <div className="mb-10 h-3 w-28 animate-pulse rounded bg-stone-200 dark:bg-stone-800" />
      <div className="h-28 w-56 animate-pulse rounded bg-stone-200 dark:bg-stone-800" />
      <div className="mt-10 space-y-3">
        <div className="h-4 w-full animate-pulse rounded bg-stone-100 dark:bg-stone-800/60" />
        <div className="h-4 w-5/6 animate-pulse rounded bg-stone-100 dark:bg-stone-800/60" />
        <div className="h-4 w-4/6 animate-pulse rounded bg-stone-100 dark:bg-stone-800/60" />
      </div>
    </div>
  );
}
