...
/**
 * Vercel Cron handler. Hit by `0 4 * * *` (every day 04:00 UTC) to
 * refresh content/_listening.json from the Spotify Web API.
 *
 * Required env vars:
 *   - SPOTIFY_CLIENT_ID
 ...
 *
 * Without those, the route returns 503 and the page falls back to
 * whatever JSON last shipped in the repo (or the empty state).
 *
 * Snapshot shape: see lib/listening.ts. The route fetches:
 *   - 6 most-recent plays
 *   - 8 short-term top tracks (the "this month" list)
 *
 * The handler-style route writes via commitRepoFile so the snapshot
 * lives in the repo and is auditable; no separate KV store needed.
 */
