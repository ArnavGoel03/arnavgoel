    ...
    <div className="space-y-8">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-full border border-stone-200 bg-white py-4 pl-14 pr-5 font-serif text-xl italic text-stone-900 placeholder:text-stone-400 focus:border-stone-900 focus:outline-none"
          type="search"
          aria-label="Search"
        />
        <kbd className="pointer-events-none absolute right-5 top-1/2 hidden -translate-y-1/2 rounded border border-stone-200 bg-stone-50 px-1.5 py-0.5 font-mono text-[10px] text-stone-500 sm:inline">
          /
        </kbd>
      </div>
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        ...
        <span className="ml-auto flex items-center text-xs text-stone-400">
          {results.length} {results.length === 1 ? "result" : "results"}
        </span>
      </div>

      {results.length === 0 ? (
        <p className="py-16 text-center font-serif text-lg italic text-stone-500">
          {query.trim()
