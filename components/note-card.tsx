...
export function NoteCard({ note }: { note: NoteSummary }) {
  return (
    <Link
      ...
      className="group flex flex-col gap-2 rounded-2xl border border-stone-200 bg-white p-6 transition-all hover:-translate-y-0.5 hover:border-stone-300 hover:shadow-sm"
    >
      ...
      <h3 className="font-serif text-xl leading-snug text-stone-900 transition-colors group-hover:text-stone-700">
        {note.title}
      </h3>
...
export function NoteRow({ note }: { note: NoteSummary }) {
  return (
    <Link
      href={`/notes/${note.slug}`}
      className="group flex items-baseline justify-between gap-6 border-b border-stone-100 py-4 last:border-none"
    >
      ...
      <time
        ...
        className="shrink-0 font-mono text-xs tabular-nums text-stone-400"
      >
